...
# the first two are user and system CPU time, elpased is wall-clock time
```
...
summary(res)$coefficient # close form estimates
```
...
r <- parLapply(cl, 1:1000, boot_fx, iris_subset)
sd(unlist(r)) # ~0.485
...
```{r}
# change function so that data is taken from global environment

...
3. `diabetes` contains two measurements of systolic and diastolic blood pressure for each participant, called `bp.1s`/`bp.2s` (systolic) and `bp.1d`/`bp.2d` (diastolic). Add new variables called `sbp` and `dbp` that are the average of the two observations for each type. Just add them up and divide by 2.
4. Select just `id` and the new variables you created.
